...
int main() {
    ...
    # ==========================
        ...
        st.info("🔜 Next Topic: Subset Construction (NFA → DFA)!")
        ...
        try:
            ...
            import random # Fix for UnboundLocalError
            nfa_engine = NFAEngine()
            
            nfa_tab1, nfa_tab2 = st.tabs(["🎲 Random NFA", "✍️ Draw Your Own"])
            
            with nfa_tab1:
                if st.button("Generate Random RE & Diagram"):
                    nfa_examples = [
                        "(a|b)*abb",
                        "a(a|b)*b",
                        "a+b*c",
            ...
            with nfa_tab2:
                user_re = st.text_input("Enter Regular Expression (e.g. `ab*+c`):", value="a+bc*")
            ...
            st.error("NFA Generator module not found. Please checks utils/nfa_generator.py")
